...
public class Main {
  ...
  public void parseFile(String fileName) throws Exception {
    ...
    while ((line = br.readLine()) != null) {
      ...
      if (!parts[0].isEmpty()) {
        if (SYMTAB.containsKey(parts[0])) SYMTAB.put(
          parts[0],
          new TableRow(parts[0], lc, SYMTAB.get(parts[0]).getIndex())
        ); else SYMTAB.put(parts[0], new TableRow(parts[0], lc, ++symIndex));
      }
      ...
      if (parts[1].equals("EQU")) {
        ...
        if (SYMTAB.containsKey(parts[0])) SYMTAB.put(
          parts[0],
          new TableRow(parts[0], loc, SYMTAB.get(parts[0]).getIndex())
        ); else SYMTAB.put(parts[0], new TableRow(parts[0], loc, ++symIndex));
      }
...
*/lc = lc + size;
  ...
  void PrintLITTAB() throws IOException {
    ...
    for (int i = 0; i < LITTAB.size(); i++) {
      ...
      bw.write(
        (i + 1) + "\t" + row.getSymbol() + "\t" + row.getAddress() + "\n"
      );
    }
    bw.close();
  }
  ...
  void printSYMTAB() throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      System.out.println(
        value.getIndex() + "\t" + value.getSymbol() + "\t" + value.getAddress()
      );
      bw.write(
        value.getIndex() +
        "\t" +
